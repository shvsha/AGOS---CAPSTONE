...
export default function Login() {
  ...
  const handleNewPasswordOpen = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setCodeError(data.code || "That code is incorrect. Please try again.")
        return
      }
    ...
    <>
      <div className="w-screen h-screen flex items-center justify-center">
        ...
        <Card className="w-full max-w-[95vw] sm:max-w-100  p-6 py-10 sm:p-10 sm:py-18 bg-[#FFFAFA] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.4)]">
              ...
              <Field className='gap-0.5'>
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className={`border focus:ring-0 bg-[#CDE3DEB0] h-8.5 pl-10 ${fieldError || loginError ? "border-red-500" : "border-none"}`}
                  />
                </div>
              </Field>
              ...
              <Field className='gap-0.5 -mt-2'>
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    autoComplete="off"
                    onChange={e => { handleCredentials(e); setFieldError(""); setLoginError("") }}
                    className={`border focus:ring-0 bg-[#CDE3DEB0] h-8.5 pl-10 pr-10 ${fieldError || loginError ? "border-red-500" : "border-none"}`}
                  />
          ...
          <CardFooter className='justify-center py-0 pb-4 flex-col bg-transparent border-none'>
            ...
            <a onClick={() => { setChangePasswordOpen(true); setEmailError(""); setResetEmail("") }} className='text-[#1565BC] text-[13px] underline mb-6 mt-2 cursor-pointer'>Forgot Password?</a>
            <hr className="border-t border-[#1565BC] w-full py-4" />
          </CardFooter>
        </Card>
      </div>
        ...
        <DialogContent className="[&>button]:hidden shadow-[0_35px_60px_-15px_rgba(0,0,0,0.4)] py-8 !max-w-[95vw] sm:!max-w-[410px]">
          ...
          <DialogHeader>
            <div className="text-center">
              <p className="text-[#122A48] font-semibold text-[23px] mb-4">Forgot Password?</p>
              <p className="text-[#122A48] px-5">Enter your email address and a code will be sent to help reset your password.</p>
            </div>
            <DialogDescription className="flex flex-col gap-5">
              <Field className='px-10'>
                <FieldLabel className='text-[#122A48] -mb-1 mt-3' htmlFor="input-field-email">Email</FieldLabel>
                <Input
                  className={`bg-[#CDE3DEB0] rounded-md text-xs !font-normal text-[#122A48] py-4.5 ${emailError ? "border-red-500" : "border-[#ccc]"}`}
              ...
              <Button
                className='!rounded-md mx-10 bg-[#122A48] shadow-[0_8px_6px_-4px_rgba(0,0,0,0.3)] py-4.5 mb-5 -mt-2 cursor-pointer font-bold'
              ...
              <div className='flex justify-center'>
                <a onClick={() => setChangePasswordOpen(false)} className='text-[#122A48] no-underline decoration-transparent cursor-pointer'>&lt; Back to Login</a>
              </div>
            </DialogDescription>
          </DialogHeader>
        </DialogContent>
      ...
      <Dialog open={checkEmailOpen} onOpenChange={handleCloseOtpDialog}>
        <DialogContent className="[&>button]:hidden py-7 px-0 !max-w-[95vw] sm:!max-w-[410px] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.4)]">
          ...
          <DialogHeader>
            <div className="text-center">
              <p className="text-[#122A48] font-semibold text-[23px]">Check your Email</p>
            ...
            <DialogDescription className="flex flex-col gap-5">
              ...
              <Button
                className='!rounded-md mx-10 bg-[#122A48] shadow-[0_8px_6px_-4px_rgba(0,0,0,0.3)] py-4.5 cursor-pointer font-bold'
              ...
              <div className='text-center -mt-1'>
                {resendCount >= RESEND_LIMIT ? (
                  <p className='text-red-500 text-xs font-medium text-center'>Too many attempts. Please try again later.</p>
                ) : cooldownSeconds > 0 ? (
                  <p className='text-gray-400 text-xs text-center'>Resend available in <span className='text-[#122A48] font-semibold'>{formatCooldown(cooldownSeconds)}</span></p>
                ) : resendSuccess ? (
                  <p className='text-green-600 text-xs font-medium text-center'>A new code has been sent to your email.</p>
                ) : isLoadingResend ? (
                  <div className="flex items-center justify-center gap-1.5">
                    <div className="w-3 h-3 border-2 border-[#122A48] border-t-transparent rounded-full animate-spin" />
                    <p className='text-[#122A48] text-xs'>Sending new code...</p>
                  </div>
                ) : (
                  <div className='flex flex-col items-center gap-1'>
                    {resendError && <p className='text-red-500 text-xs flex items-center justify-center gap-1'><span>⊙</span>{resendError}</p>}
                    <p className='text-black text-xs text-center'>
                      Didn't get any code?{" "}
                      <a onClick={handleResendCode} className='text-[#122A48] cursor-pointer underline'>Click to resend</a>
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className='flex justify-center'>
                <a onClick={handleCloseOtpDialog} className='text-[#122A48] no-underline decoration-transparent cursor-pointer'>&lt; Back to Login</a>
              </div>
            </DialogDescription>
          </DialogHeader>
        </DialogContent>
          ...
          {isLoadingReset ? (
            <div className="flex flex-col items-center justify-center py-10 gap-5">
              <div className='bg-[#CDE3DE] py-7 px-7 rounded-2xl flex justify-center mb-3'>
                <div className="w-13 h-13 border-4 border-[#122A48] border-t-transparent rounded-full animate-spin" />
              </div>
              <div className='text-center'>
                <p className='text-[#122A48] font-bold text-xl'>Updating Password</p>
                <p className='text-xs mx-5 mt-4 text-gray-500'>Please wait while we securely update your password.</p>
              </div>
              <div className="flex gap-2 mt-6">
                <div className="w-2.5 h-2.5 bg-[#122A48] rounded-full animate-bounce [animation-delay:0ms]" />
                <div className="w-2.5 h-2.5 bg-[#122A48] rounded-full animate-bounce [animation-delay:150ms]" />
                <div className="w-2.5 h-2.5 bg-[#122A48] rounded-full animate-bounce [animation-delay:300ms]" />
              </div>
            </div>
          ) : (
            <>
              ...
              <DialogHeader>
                <div className='text-center'>
                  <p className='text-[#122A48] font-semibold text-[23px] mb-2'>Set a new password</p>
                  <p className='text-[#122A48] px-5'>Your new password must be different from previously used passwords.</p>
                </div>
                <DialogDescription className='flex flex-col gap-5'>
                  <Field className='px-10 -mb-3'>
                    <FieldLabel className='text-[#122A48] -mb-1 mt-3' htmlFor="new-password">New Password</FieldLabel>
                    <div className="relative">
                      <Input
                        className="bg-[#CDE3DEB0] rounded-md border-[#ccc] text-xs !font-normal text-[#122A48] py-4.5 pr-10"
                        id="new-password"
                  ...
                  <Field className='px-10'>
                    <FieldLabel className='text-[#122A48] -mb-1' htmlFor="confirm-password">Confirm New Password</FieldLabel>
                    <div className="relative">
                      <Input
                        className="bg-[#CDE3DEB0] rounded-md border-[#ccc] text-xs !font-normal text-[#122A48] py-4.5 pr-10"
                        id="confirm-password"
                    ...
                    <div className='flex flex-col gap-1 mt-2'>
                      {validationError && <p className='text-red-500 text-xs mb-1'>Please satisfy all requirements before proceeding.</p>}
                      {[
                  ...
                  <div className='flex justify-center pb-3'>
                    <a onClick={() => setNewPasswordOpen(false)} className='text-[#122A48] cursor-pointer'>&lt; Back to Login</a>
                  </div>
                </DialogDescription>
              </DialogHeader>
            </>
      ...
      <Dialog open={successChangePasswordOpen} onOpenChange={setSuccessChangePasswordOpen}>
        <DialogContent className="[&>button]:hidden py-7 px-0 !max-w-[95vw] sm:!max-w-[410px] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.4)]">
          ...
          <DialogHeader>
            <div className='text-center'>
              <p className='text-[#122A48] font-semibold text-[23px] mb-2'>Password Reset!</p>
              <p className='text-[#122A48] px-5 mb-5'>You've successfully created a new password, click below to login.</p>
            </div>
            <DialogDescription className='flex flex-col gap-5'>
              <Button
                className='!rounded-md mx-10 bg-[#122A48] shadow-[0_8px_6px_-4px_rgba(0,0,0,0.3)] py-4.5 mb-5 cursor-pointer font-bold'
        ...
        <DialogContent className="[&>button]:hidden py-7 px-0 !max-w-[95vw] sm:!max-w-[410px] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.4)]">
          ...
          <DialogHeader>
            ...
            <DialogDescription className='flex flex-col px-10 mt-4'>
              <Button
                className='!rounded-md bg-[#122A48] shadow-[0_8px_6px_-4px_rgba(0,0,0,0.3)] py-4.5 cursor-pointer font-bold'
